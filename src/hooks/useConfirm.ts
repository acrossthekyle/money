  ...
  const confirm = async (config: Config) => {
    return withReactContent(Swal).fire({
      ...
      customClass: {
        ...
        cancelButton: '!inline-flex !items-center !justify-center !rounded-full !bg-(--background) !border !border-current/22.5 !text-(--foreground) !text-xs !h-9 !w-9 !transition-all !motion-safe:duration-300 hover:!border-current/62.5',
        confirmButton: '!inline-flex !items-center !justify-center !rounded-full !border !border-current/22.5 !bg-(--foreground) !text-(--background) !text-xs !h-9 !w-9 !transition-all !motion-safe:duration-300 hover:!bg-(--foreground)/75',
