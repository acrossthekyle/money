...
import type { UseConfirmConfig } from '@/types';

export function useConfirm() {
  const confirm = async (config: UseConfirmConfig) => {
    return withReactContent(Swal).fire({
      ...
      customClass: {
        ...
        input: '!flex-col !items-start !gap-4 !w-full !col-start-1 !col-end-3 !text-xs !text-(--foreground) !mt-0 !mb-5 !mx-1 [&_label]:!flex [&_label]:!gap-1 [&_label]:!items-center !bg-(--background)',
