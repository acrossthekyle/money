  ...
  const confirm = async (config: Config) => {
    return withReactContent(Swal).fire({
      ...
      customClass: {
        ...
        popup: '!rounded-md !bg-(--background) !border !border-(--foreground)/12.5 text-left text-(--foreground) w-full !max-w-md !grid-cols-none !pb-0 !overflow-hidden',
        ...
        input: '!flex-col !items-start !gap-4 !text-xs !mt-6 !mb-5 !mx-1 [&_label]:!flex [&_label]:!gap-1 [&_label]:!items-center',
